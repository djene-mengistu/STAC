...
def load_image_label_list_from_npy(img_name_list, label_file_path=None):
    if label_file_path is None:
        label_file_path = '/data/djene/djene/MCTCon/dagm_seg/dagm_f_labels.npy'
...
class NEUDataset(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None, gen_attn=False):
        ...
        img_name_list_path = os.path.join(img_name_list_path, f'{"dagm_f_samples" if train else "test"}.txt')   #Modified
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(os.path.join(self.voc12_root, 'xJPEGImages', name + '.jpg')).convert("RGB")
        sal = PIL.Image.open(os.path.join(self.voc12_root, 'xSALmapsALL', name + '.PNG'))#.convert("L") # 'convert 'RGB' changes the saliency map to 3 channel
...
class NEUDatasetMS(Dataset):
    def __init__(self, img_name_list_path, voc12_root, scales, train=False, transform=None, gen_attn=True, unit=1):
        ...
        # else:
        #     img_name_list_path = os.path.join(img_name_list_path, f'{"test"}.txt')
        img_name_list_path = os.path.join(img_name_list_path, f'{"dagm_f_samples" if train else "train"}.txt')  #Modified
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
        self.voc12_root = voc12_root
        self.transform = transform
