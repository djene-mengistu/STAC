import sys
sys.path.append("/data/djene/djene/EPS_CAM")
import random
import os.path
import PIL.Image
import numpy as np
...
def load_img_label_list_from_npy(img_name_list, dataset, tasks):
    if tasks=='train':
        cls_labels_dict = np.load(f'/data/djene/djene/EPS_CAM/metadata/neu_seg/cls_labels.npy', allow_pickle=True).item()
    else:
        cls_labels_dict = np.load(f'/data/djene/djene/EPS_CAM/metadata/neu_seg/cls_labels_test.npy', allow_pickle=True).item()
    return [cls_labels_dict[img_name] for img_name in img_name_list]


def get_saliency_path(img_name, saliency_root='SALmapsALL'):
...
class ClassificationDataset(ImageDataset):
    ...
    def __init__(self, dataset, img_id_list_file, img_root, transform=None):
        ...
        self.label_list = load_img_label_list_from_npy(self.img_id_list, dataset, tasks = 'train')
...
class ClassificationDatasetWithSaliency(ImageDataset):
    ...
    """
    def __init__(self, dataset, img_id_list_file, img_root, saliency_root=None, crop_size=None, resize_size=(224, 224)):
        super().__init__(dataset, img_id_list_file, img_root, transform=None)
        self.saliency_root = saliency_root
        self.crop_size = crop_size
        self.resize_size = resize_size

        self.resize = transforms.Resize((224, 224))#RandomResizeLong(resize_size[0], resize_size[1]) #transforms.Resize((224, 224)) #
    ...
    def __getitem__(self, idx):
        ...
        # saliency = PIL.Image.open(get_saliency_path(img_id, self.saliency_root)).convert("L")
