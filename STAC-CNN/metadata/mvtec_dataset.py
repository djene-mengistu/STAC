...
def load_img_label_list_from_npy(img_name_list, dataset, tasks):
    if tasks=='train':
        cls_labels_dict = np.load(f'/data/djene/djene/EPS_CAM/mvtec/carpet/cls_labels.npy', allow_pickle=True).item()
    else:
        cls_labels_dict = np.load(f'/data/djene/djene/EPS_CAM/mvtec/carpet/cls_labels.npy', allow_pickle=True).item()
    return [cls_labels_dict[img_name] for img_name in img_name_list]


def get_saliency_path(img_name, saliency_root='rSALmapsALL'):
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(os.path.join(self.img_root, img_id + '.png')).convert("RGB")
