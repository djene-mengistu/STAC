...
# categories = ['background','Blowhole','Break','Crack', 'Fray','Uneven']
def compare(start,step,TP,P,T,input_type,threshold,predict_folder, gt_folder, sal_folder, name_list, num_cls=6):
...
def do_python_eval(predict_folder, gt_folder, sal_folder, name_list, num_cls=6, input_type='png', threshold=1.0, printlog=False):
    TP = []
    P = []
    T = []
    for i in range(num_cls):
    ...
    # p_list.append(p)
    for i in range(8):
        p = multiprocessing.Process(target=compare, args=(i,8,TP,P,T,input_type,threshold, args.pred_dir, args.gt_dir, args.sal_dir, name_list, args.num_classes))
        p.start()
        p_list.append(p)
    for p in p_list: 
        p.join()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sal_dir", default='/data/djene/djene/data/MTD_VOC/SALmapsALL', type=str)
    ...
    df = pd.read_csv(args.datalist, names=['filename'], dtype=str) #Modified for dtype to keep starting zeros
    name_list = df['filename'].values
    
    loglist = do_python_eval(args.pred_dir, args.gt_dir, args.sal_dir, name_list, args.num_classes, args.type, args.t, printlog=True)
    ...
    if not args.curve:
        ...
        for i in range(args.start, args.end, 3):
            t = i/100.0
            loglist = do_python_eval(args.pred_dir, args.gt_dir, args.sal_dir, name_list, args.num_classes, args.type, t, printlog=True)
        ...
        writelog(args.save_path, {'mIoU':l, 'Best mIoU': max_mIoU, 'Best threshold': best_thr}, args.comment)
