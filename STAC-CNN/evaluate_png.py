...
if __name__ == '__main__':
    # get arguments
    args = parse_args()
    idx2num, idx2label = get_labels(os.path.join('/data/djene/djene/EPS_mod/metadata', args.dataset, 'labels.txt'))

    mIOU = IOUMetric(num_classes=args.num_classes)

    img_ids = open(args.datalist).read().splitlines()

    postfix = '.png' #Change accordingly
