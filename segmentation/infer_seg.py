...
classes = np.array(('background',  'inclusion', 'patches', 'scratches'))
# classes = np.array(('background', 'class1', 'class2', 'class6', 'class7', 'class9', 'class10'))
# classes = np.array(('background','Blowhole','Break','Crack', 'Fray','Uneven'))

def crf_postprocess(pred_prob, ori_img):
    crf_score = imutils.crf_inference_inf(ori_img, pred_prob, labels=7)
    return crf_score
...
if __name__ == '__main__':
    ...
    # NET = Transformer_model('MiT-B0', 6) #
    # NET.init_pretrained('/data/djene/djene/MCTCon/seg/network/custom_models/weights/mit_b0.pth')
    # model = NET
    # # model.load_state_dict(torch.load(args.weights))
    ...
    checkpoint_0 = torch.load(args.weights)
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(img_list))):
            ...
            # Resize the image to 208x208
            img_temp = cv2.resize(img_temp, (224, 224)) #Only for DLV3+            
            ...
            gt = gt.resize((224, 224), Image.NEAREST)  # Use NEAREST for segmentation masks #Only for DLV3+   
            gt = np.asarray(gt)
