...
def load_image_label_list_from_npy(img_name_list, label_file_path=None):
    if label_file_path is None:
        label_file_path = '/data/djene/djene/MCTCon/mtd_seg/all_freedef_labels.npy'
...
class NEUDataset(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None, gen_attn=False):
        ...
        img_name_list_path = os.path.join(img_name_list_path, f'{"def_train_samples" if train else "def_test_samples"}.txt')   #Modified
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(os.path.join(self.voc12_root, 'gJPEGImages', name + '.jpg')).convert("RGB")
        sal = PIL.Image.open(os.path.join(self.voc12_root, 'gSALmapsALL', name + '.png'))#.convert("L") # 'convert 'RGB' changes the saliency map to 3 channel
        ...
        sal_transform = transforms.Compose([
            # rfh,
            # rfv,
            transforms.Resize((224, 224)),  # Resize to match the input size expected by the backbone
            transforms.ToTensor()
        ])
